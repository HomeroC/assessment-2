...
const canWeDeliver = zipCode => {
    if(deliveryAreaZipCodes.includes(zipCode)){
        console.log("Yes, we can deliver to you!")
    } 
    else {
        console.log("Sorry, we don't deliver in your area.")
    }
}
canWeDeliver(85205)
canWeDeliver(77521)
...
const canWeDeliverTwo = (zipCode) => {
    for(let i = 0; i < deliveryAreaZipCodes.length; i++){
        if(zipCode === deliveryAreaZipCodes[i]){
           return console.log("Yes we deliver!")
        }
        else {
            return console.log("Sorry we don't deliver in your area.")
        }
    }
    
}

canWeDeliverTwo(77521)
...
let titleName = deals[0].title.replace("15", "10")
...
let desName = deals[1].desc.trim().replace("March", "April")
console.log(desName)